...
impl Calendar {
    fn check_availability_permissions(
        &self,
        caller_id: &str,
    ...
    fn update_availabilities(&mut self, availabilities: Vec<Availability>) {
        for new_availability in availabilities {
            if let Some(index) = self
                .availabilities
                .iter()
                .position(|a| a.id == new_availability.id)
            {
...
fn update_calendar(calendar_id: String, actions: CalendarActions) -> Result<Calendar, String> {
    ...
    if caller_id == calendar.owner {
        ...
        calendar
            .availabilities
            .retain(|availability| !actions.delete_availabilities.contains(&availability.id));
    }
...
fn add_google_calendar_id(calendar_id: String, ids: Vec<String>) -> Result<String, String> {
    ...
    if caller_id != calendar.owner {
        return Err(
            "Permission denied: Only calendar owner can add Google calendar IDs".to_string(),
        );
    }
