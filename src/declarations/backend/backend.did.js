export const idlFactory = ({ IDL }) => {
  ...
  const Result_4 = IDL.Variant({ Ok: GetTransactions, Err: Error });
  ...
  const FileNode = IDL.Record({
    ...
    author: IDL.Text,
    users_permissions: IDL.Vec(IDL.Tuple(IDL.Principal, ShareFilePermission)),
  ...
  const InitialData = IDL.Record({
    FilesContents: IDL.Opt(IDL.Vec(IDL.Tuple(IDL.Text, IDL.Vec(ContentNode)))),
  ...
  return IDL.Service({
    ...
    get_operation_cycles: IDL.Func([IDL.Text], [Result_1], []),
    get_owners: IDL.Func([], [IDL.Vec(IDL.Principal)], ["query"]),
    get_post: IDL.Func([IDL.Text], [Result_10], ["query"]),
    get_posts: IDL.Func([IDL.Nat64, IDL.Nat64], [IDL.Vec(PostUser)], ["query"]),
    ...
    get_total_cycles: IDL.Func([], [Result_1], []),
    ...
    record_cycles: IDL.Func([IDL.Text, IDL.Nat64], [Result], []),
    ...
    send_message: IDL.Func([IDL.Opt(IDL.Principal), Message], [Result_1], []),
