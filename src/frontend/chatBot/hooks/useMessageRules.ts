...
// Re-export types
export type { MessageContent, MessageRule } from "../services/MessageRulesService";

export const useMessageRules = () => {
  // Combine selectors to reduce re-renders
  const { calendar, currentJobId, jobs, jobSearchStage } = useSelector((state: RootState) => ({
  ...
  return useMemo(() => ({
    ...
    getMessagesByType: messageRulesService.getMessagesByType.bind(messageRulesService),
    getTriggeredMessages: messageRulesService.getTriggeredMessages.bind(messageRulesService),
    getAutomaticMessage: messageRulesService.getAutomaticMessage.bind(messageRulesService),
    getOnboardingMessage: messageRulesService.getOnboardingMessage.bind(messageRulesService),
    shouldShowWelcomeMessage: messageRulesService.shouldShowWelcomeMessage.bind(messageRulesService),
  }), [messageRulesService]);
};