...
const useInfiniteScroll = (chat, backendActor, dispatch) => {
  ...
  const handleScroll = useCallback(async (container) => {
...
const useChatOperations = (chat, backendActor, onUpdateChat, dispatch, onClose) => {
  const [isSending, setIsSending] = useState(false);
  const [isSaving, setIsSaving] = useState(false);
  const [saveSuccess, setSaveSuccess] = useState(false);
  const saveSuccessTimeout = useRef(null);

  const handleSendMessage = useCallback(async (messageText, onSendMessage, profile) => {
  ...
  const handleSaveChat = useCallback(async (updatedChat) => {
    setIsSaving(true);
    try {
      const formattedChat = {
        ...updatedChat,
...
const renderSenderName = (sender, profile, allFriends) => {
  ...
  return senderStr === profile?.id
    ? "You"
    : allFriends.find((u) => u.id === senderStr)?.name || senderStr.slice(8, 16);
};
...
const ChatHeader = memo(({ 
  ...
  const chatDisplayName = isPrivateChat
    ? renderSenderName(chat.members.find((m) => m.toText() !== profile?.id), profile, [])
    : chat.name;
...
const ChatWindow = memo(({
  ...
  const chatOperations = useChatOperations(chat, backendActor, onUpdateChat, dispatch, onClose);
  ...
  const handleSendMessageWrapper = useCallback(async (messageText) => {
    const success = await chatOperations.handleSendMessage(messageText, onSendMessage, profile);
    if (success) {
      infiniteScroll.setIsScrolledToBottom(true);
      setTimeout(scrollToBottom, 100);
    }
    return success;
  }, [chatOperations.handleSendMessage, onSendMessage, profile, infiniteScroll.setIsScrolledToBottom, scrollToBottom]);

  const handleSaveChatWrapper = useCallback(async (updatedChat) => {
  ...
  return (
    <Dialog
      ...
      PaperProps={{
        sx: {
          height: isMinimized ? 'auto' : { xs: '90vh', sm: '70vh', md: '600px' },
          maxHeight: '90vh',
          display: "flex",
          flexDirection: "column",
