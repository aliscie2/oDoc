...
const StyledTooltip = styled(({ className, ...props }: TooltipProps) => (
  ...
  [`& .${tooltipClasses.tooltip}`]: {
    ...
    background:
      "linear-gradient(135deg, rgba(15, 15, 25, 0.98) 0%, rgba(25, 25, 40, 0.95) 100%)",
  },
...
const getRewardTier = (score: number) => {
  if (score >= 4.5)
    return { percentage: 0.2, level: "Platinum", icon: "💎", emoji: "👑" };
  if (score >= 4)
    return { percentage: 0.15, level: "Gold", icon: "🏆", emoji: "⭐" };
  if (score >= 3.5)
    return { percentage: 0.1, level: "Silver", icon: "🥈", emoji: "🌟" };
  if (score >= 3)
    return { percentage: 0.05, level: "Bronze", icon: "🥉", emoji: "⚡" };
