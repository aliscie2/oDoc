...
function useParser(props: ParserProps) {
  ...
  const all_users: Array<User> = all_friends
    ? [profile, ...all_friends]
    : [profile];
  ...
  function updatePromises() {
    ...
    if (main_contract) {
      ...
      new_promises.forEach((newPromise) => {
        const existingPromiseIndex = main_contract.promises.findIndex(
          (p) => p.id === newPromise.id,
        );
        if (existingPromiseIndex > -1) {
          // Check if the promise has been changed
          if (
            JSON.stringify(main_contract.promises[existingPromiseIndex]) !==
            JSON.stringify(newPromise)
          ) {
            changes = true;
  ...
  return {
    evaluate,
    addVarsToParser,
    promises: Array.from(ref.current.values()),
  };
}
