...
import { ClientSideRowModelModule, colorSchemeDarkBlue, colorSchemeLightWarm, ModuleRegistry, NumberEditorModule, SelectEditorModule, TextEditorModule, themeBalham, ValidationModule } from "ag-grid-community";
import { CellSelectionModule, ClipboardModule, ColumnMenuModule, ContextMenuModule, StatusBarModule } from "ag-grid-enterprise";
...
const PAYMENT_STATUSES = { None: null, RequestCancellation: null, Released: null, Objected: "", Confirmed: null, ConfirmedCancellation: null, ApproveHighPromise: null, HighPromise: null };

// Hooks
const useContractData = (contractId, contracts) => useMemo(() => contracts[contractId] || { promises: [], payments: [], contracts: [] }, [contractId, contracts]);

const useDataType = (contractId) => {
  const [dataType, setDataType] = useState(() => localStorage.getItem(`contract-${contractId}-dataType`) || DATA_TYPES.AGREEMENT);
  useEffect(() => localStorage.setItem(`contract-${contractId}-dataType`, dataType), [contractId, dataType]);
  return [dataType, setDataType];
};

const useColumnConfig = (profile, all_friends, wallet) => useMemo(() => ({
  amount: {
    headerName: "Amount", type: "numericColumn", cellEditor: "agNumberCellEditor", cellEditorParams: { min: 0 },
    valueFormatter: (p) => `${p.value?.toLocaleString() || 0}`,
    validation: (params) => params.newValue > wallet.balance ? "Error: Not enough balance" : null,
  },
  status: {
  ...
  sender: {
    ...
    valueFormatter: (p) => {
      ...
      return senderId === profile.id ? "You" : all_friends?.find((u) => u.id === senderId)?.name || "Unknown";
    },
  },
  receiver: {
    ...
    valueGetter: (p) => {
      ...
      return receiverId === profile.id ? "You" : all_friends?.find((u) => u.id === receiverId)?.name || "Anonymous";
    },
    valueSetter: (p) => {
    ...
    validation: (params) => params.data.sender.toString() === params.receiver.toString() ? "Error: You can't send to yourself" : null,
  },
  date_created: {
...
const useGridData = (currentContract, dataType, columnConfig) => useMemo(() => {
  ...
  const sourceData = dataType === DATA_TYPES.PROMISE ? currentContract.promises : currentContract.payments;
  const rowData = sourceData?.map((item) => {
  ...
  const baseColumns = Object.entries(columnConfig).map(([field, config]) => ({
    field, ...config, editable: dataType === DATA_TYPES.PROMISE && config.editable !== false,
    cellStyle: (params) => config.validation?.(params) ? { backgroundColor: "#ffebee" } : {},
  }));
  
  const dynamicColumns = cellFields.map((field) => ({
    field, headerName: field.charAt(0).toUpperCase() + field.slice(1).replace(/_/g, " "),
    editable: dataType === DATA_TYPES.PROMISE, cellEditor: "agTextCellEditor",
  }));
...
const DataTypeSelector = memo(({ currentType, onTypeChange, contractData, onAddTable, onRename, onSwitchToAgreement, onDeleteTable }) => {
  const getCurrentName = () => {
    ...
    return contractData.contracts?.find(t => t.id === currentType)?.name || 'Unnamed Table';
  };

  const handleRename = () => {
    const currentName = getCurrentName();
    const isContract = [DATA_TYPES.PROMISE, DATA_TYPES.PAYMENT, DATA_TYPES.AGREEMENT].includes(currentType);
    const newName = prompt(`Rename ${isContract ? 'Contract' : 'Table'}:`, currentName);
...
const AgGridContainer = memo(({ contractId, dataType, currentContract, gridData, onCellValueChanged, getContextMenuItems, isDarkMode, statusBarProps }) => {
  ...
  const noRowsMessages = {
    ...
    [DATA_TYPES.PAYMENT]: "Payments will appear here when promises are released",
  ...
  return (
    <div style={{ height: `${gridHeight}px`, width: "100%" }}>
      <AgGridReact
        ...
        statusBar={{ statusPanels: [{ statusPanel: DataTypeSelector, statusPanelParams: statusBarProps, align: "center" }] }}
        cellSelection={true}
...
const CustomContractViewer = memo(({ contractId }) => {
  ...
  const handleAddTable = useCallback(() => {
    ...
    dispatch({
      ...
      table: {
        id: `table_${Date.now()}`, name, date_created: Date.now() * 1e6, creator: Principal.fromText(profile.id), rows: [],
        columns: [{ id: `col_${Date.now()}`, field: "untitled", name: "Untitled", column_type: "string", filters: [], permissions: [{ AnyOneView: null }], formula_string: "", editable: true, deletable: false }],
      },
    });
  }, [dispatch, contractId, profile.id]);

  const handleRename = useCallback((currentType, newName) => {
    const actionType = [DATA_TYPES.PROMISE, DATA_TYPES.PAYMENT].includes(currentType) ? "RENAME_SMART_CONTRACT" : "RENAME_TABLE";
  ...
  const onCellValueChanged = useCallback(async (params) => {
    ...
    if (isTable) {
      ...
      const updatedRow = {
        id: data.id,
        cells: table?.columns?.map((col) => ({ id: `${data.id}_${col.field}`, field: col.field, value: data[col.field] || "" })) || [],
      };
  ...
  }, [currentContract, contractId, dispatch, enqueueSnackbar, columnConfig, dataType]);
  ...
  const getContextMenuItems = useCallback((params) => {
    ...
    const tableActions = {
      "Add Row": (params) => {
        const table = currentContract.contracts?.find((t) => t.id === dataType);
        const insertIndex = params?.node?.rowIndex !== undefined ? params.node.rowIndex + 1 : table?.rows?.length || 0;
        const newRow = {
          id: `row_${Date.now()}`,
          cells: table?.columns?.map((col) => ({ id: `cell_${Date.now()}_${col.field}`, field: col.field, value: "" })) || [],
        };
        dispatch({ type: "ADD_ROW", contract_id: contractId, table_id: dataType, row: newRow, insertIndex });
      },
      "Add Column": (params) => {
        const field = prompt("Column name:");
        if (!field) return;
        const table = currentContract.contracts?.find((t) => t.id === dataType);
        const insertIndex = params?.column ? table?.columns?.findIndex((col) => col.field === params.column.getColId()) + 1 : undefined;
        const newColumn = {
          id: `col_${Date.now()}`, field, name: field, column_type: "string", filters: [],
          permissions: [{ AnyOneView: null }], formula_string: "", editable: true, deletable: true,
        };
        dispatch({ type: "ADD_COLUMN", contract_id: contractId, table_id: dataType, column: newColumn, insertIndex });
      },
    };
    ...
    if (params.node) {
      ...
      items.push({ name: `Delete ${dataType === DATA_TYPES.PROMISE ? "Promise" : "Row"}`, action: deleteAction });
    }
      ...
      items.push({
        ...
        action: () => {
          ...
          if (newName && newName !== params.column.getColId() && column) {
            dispatch({ type: "UPDATE_COLUMN", contract_id: contractId, table_id: dataType, column: { ...column, field: newName, name: newName } });
          }
        },
      });

      if (column?.deletable) {
        items.push({ name: "Delete Column", action: () => dispatch({ type: "DELETE_COLUMN", contract_id: contractId, table_id: dataType, column_id: column.id }) });
      }
    }
