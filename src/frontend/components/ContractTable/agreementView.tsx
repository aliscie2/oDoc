...
const PromiseCard = memo(({ promise, isExpanded, onToggle, isDarkMode }) => {
  ...
  const getUserName = useCallback(
    (principalId) => {
      ...
      return id === profile.id
        ? "You"
        : all_friends?.find((u) => u.id === id)?.name || "Unknown";
    },
  ...
  const handleKeyChange = useCallback(
    (oldKey, newKey) => {
      ...
      const cells =
        promise.cells?.map((c) =>
          c.field === oldKey
            ? { ...c, field: newKey, id: `${promise.id}_${newKey}` }
            : c,
        ) || [];
      updatePromise({ cells });
      setEditingKey(null);
  ...
  const handleCellChange = useCallback(
    (field, value) => {
      const cells = [
        ...(promise.cells?.filter((c) => c.field !== field) || []),
  ...
  const handleAddCell = useCallback(() => {
    const existingUntitled =
      promise.cells?.filter((c) => c.field.startsWith("untitled")).length || 0;
    const field =
      existingUntitled === 0 ? "untitled" : `untitled_${existingUntitled}`;
  ...
  return (
    ...
    >
      ...
      >
        <Typography
          variant="caption"
          sx={{ color: statusColor, fontWeight: "bold", minWidth: 80 }}
        >
        ...
        >
          ...
          <Stack spacing={1} sx={{ mb: 1.5 }}>
            <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
              <Typography
                variant="caption"
                sx={{ minWidth: 80, color: isDarkMode ? "#aaa" : "#666" }}
              >
                Status:
              </Typography>
              <Select
                  ...
                  >
                    {editingKey === cell.field ? (
                      <TextField
                        ...
                        onBlur={(e) =>
                          handleKeyChange(cell.field, e.target.value)
                        }
                        onKeyDown={(e) => {
                          if (e.key === "Enter")
                            handleKeyChange(cell.field, editingValue);
                          if (e.key === "Escape") {
                            setEditingKey(null);
                            setEditingValue("");
                          }
                      ...
                      <Typography
                        ...
                        onClick={() => {
                          setEditingKey(cell.field);
                          setEditingValue(cell.field);
                        }}
                      >
                    ...
                    <TextField
                      value={cell.value || ""}
                      onChange={(e) =>
                        handleCellChange(cell.field, e.target.value)
                      }
                      size="small"
                      fullWidth
