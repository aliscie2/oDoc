      ...
      <ListItemText
        primary={
          <TextField
            ...
            slotProps={{ input: { sx: { fontWeight: 600, fontSize: "0.95rem" } } }}
          />
        }
        secondary={
          <TextField
            ...
            {...FIELD_CONFIGS.standardTextField}
            slotProps={{ input: { sx: { color: "text.secondary", fontSize: "0.875rem" } } }}
          />
        }
      />
    </ListItem>
...
const PromiseCardHeader = memo<{
...
}>(({ promise, isExpanded, onToggle, onStatusChange, onAmountChange, onReceiverChange, isEditable, viewMode = "promises", contract }) => {
  const getUserData = useUserData();
  const { profile, all_friends } = useSelector((state: AppState) => state.filesState);
  const { notifications } = useSelector((state: AppState) => state.notificationState);
  ...
  const handleHeaderClick = async (e: React.MouseEvent) => {
    ...
    if (unseenNotification && backendActor && !isNotificationAlreadyCalled(unseenNotification.id)) {
      try {
  ...
  const shouldShowReceiver = viewMode === "promises" || receiverData.name !== "Unknown" || (promise.amount && promise.amount > 0);
  const shouldShowAmount = viewMode === "promises" || receiverData.name !== "Unknown" || (promise.amount && promise.amount > 0);
    ...
    >
      <Box sx={{ display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 2 }}>
        ...
        <Box sx={{ display: "flex", alignItems: "center", gap: 2, flex: 1, minWidth: "200px" }}>
          <Typography variant={isExpanded ? "h6" : "body1"} fontWeight={700} sx={{ color: "primary.main" }}>
            {firstCellTitle}
          </Typography>
          {isExpanded ? (
            <Box className="editable-field" onClick={(e) => e.stopPropagation()}>
              <TextField
                label="Status"
                select
                size="small"
        ...
        {/* Sender and Receiver */}
        <Box sx={{ display: "flex", alignItems: "center", gap: 1, flex: 1, justifyContent: "center", minWidth: "200px" }}>
          <UserAvatarMenu user={senderData.user} />
          <Typography variant={isExpanded ? "body2" : "caption"} fontWeight={600}>
            {senderData.name}
          </Typography>
        ...
        {/* Amount */}
        {shouldShowAmount && (
          <Box sx={{ display: "flex", alignItems: "center", gap: 1, flex: 1, justifyContent: "flex-end", minWidth: "150px" }}>
            ...
            {isExpanded ? (
              ...
              <Typography
                ...
                sx={{ fontWeight: 700, color: "success.main", fontFamily: "monospace" }}
              >
...
const ContractHeader = memo<{
  contract: CustomContract;
  viewMode: "promises" | "payments";
  onViewModeChange: (event: React.MouseEvent<HTMLElement>, newMode: "promises" | "payments" | null) => void;
  onAddPromise: () => void;
...
}>(({ contract, viewMode, onViewModeChange, onAddPromise, onSwitchToTable, isEditable, isOnContractPage, hideReceivedPromises, onHideReceivedPromisesChange }) => {
  const { profile } = useSelector((state: RootState) => state.filesState);
  const dispatch = useDispatch();
  const navigate = useNavigate();
  const theme = useTheme();
  const isMobile = useMediaQuery(theme.breakpoints.down(RESPONSIVE_STYLES.mobileBreakpoint));
  ...
  return (
    ...
    >
      {/* Left Section */}
      <Box sx={{ display: "flex", alignItems: "center", gap: { xs: 0.5, sm: 1 }, flex: { xs: "1 1 100%", md: "1 1 auto" }, order: { xs: 1, md: 1 }, minWidth: 0 }}>
        <CopyButton
        ...
        {!isOnContractPage && (
          <Tooltip title="Open full view">
            <IconButton
              onClick={() => navigate(createShortContractUrl({
                id: contract.id,
                owner: contract.creator?.toString() || "",
      ...
      >
        <ToggleButton value="promises">
          <HandshakeIcon sx={{ fontSize: RESPONSIVE_STYLES.iconSize, mr: 0.5 }} />
          {isMobile ? `P (${contract.promises?.length || 0})` : `Promises (${contract.promises?.length || 0})`}
        </ToggleButton>
        <ToggleButton value="payments">
          <PaymentIcon sx={{ fontSize: RESPONSIVE_STYLES.iconSize, mr: 0.5 }} />
          {isMobile ? `Pay (${contract.payments?.length || 0})` : `Payments (${contract.payments?.length || 0})`}
        </ToggleButton>
      </ToggleButtonGroup>

      {/* Right Section */}
      <Box sx={{ display: "flex", gap: { xs: 0.5, sm: 1 }, alignItems: "center", order: { xs: 2, md: 3 }, flex: { xs: "0 0 auto", md: "0 0 auto" } }}>
        {viewMode === "promises" && contract.creator?.toString() !== profile.id && onHideReceivedPromisesChange && (
          <FormControlLabel
            control={
              <Switch
                checked={hideReceivedPromises}
                onChange={(e) => onHideReceivedPromisesChange(e.target.checked)}
                size="small"
              />
            }
            label="my promises"
...
const PromiseCard = memo<{
  ...
  viewMode?: "promises" | "payments";
  contract: CustomContract;
}>(({ promise, isExpanded, onToggle, isDarkMode, isEditable, viewMode = "promises", contract }) => {
  const { updatePromise, handleStatusChange, handleAmountChange, handleReceiverChange, handleDeletePromise } = usePromiseActions(promise, isEditable);
                  ...
                  {promise.cells.map((cell, index) => (
                    <Box key={cell.id}>
                      <ConditionCell
                        ...
                        isEditable={isEditable}
                      />
                      {index < promise.cells.length - 1 && <Divider variant="inset" component="li" />}
                    </Box>
                  ))}
                </List>
              ) : (
                <Box sx={{ textAlign: "center", py: 3, color: "text.secondary", mb: 3 }}>
...
const AgreementView = memo<{
  ...
  if (!contract?.id) {
    return (
      <Box sx={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", height: "200px", gap: 2, color: "text.secondary" }}>
        <DescriptionIcon sx={{ fontSize: 48, opacity: 0.3 }} />
        <Typography variant="h6">Contract not found</Typography>
        <Typography variant="body2">This contract may not exist or failed to load.</Typography>
        <Button onClick={onSwitchToTable} variant="outlined">Switch to Table View</Button>
      </Box>
    );
  }

  return (
    <Box sx={{ p: RESPONSIVE_STYLES.containerPadding, maxHeight: "70vh", overflowY: "auto", bgcolor: "background.default" }}>
      <ContractHeader
        contract={contract}
        viewMode={viewMode}
