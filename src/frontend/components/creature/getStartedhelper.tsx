...
const TypewriterText = ({ text, speed = 50, onComplete, variant = "body1" }) => {
...
const ChatBubble = ({ children, isVisible, jellyfishPos, containerSize, onClose }) => {
  const theme = useTheme();
  const [bubblePosition, setBubblePosition] = useState({ top: 0, left: 0, position: 'top' });
  ...
  useEffect(() => {
    ...
    if (spaceTop < bubbleHeight + margin && spaceBottom > bubbleHeight + margin) {
      // Not enough space above, show below
      position = 'bottom';
      top = jellyfishPos.y + jellyfishBuffer; // Below jellyfish with buffer
    } else if (spaceTop < bubbleHeight + margin && spaceBottom < bubbleHeight + margin) {
    ...
    const handleMouseMove = (e) => {
      ...
      if (jellyfishRect) {
        ...
        if (distance < 100) {
          ...
          // Keep within bounds
          const boundedX = Math.max(100, Math.min(containerSize.width - 100, newX));
          const boundedY = Math.max(100, Math.min(containerSize.height - 100, newY));
    ...
    const animateEyes = () => {
      ...
      if (step === 2) {
        targetElements = [
          ...
          ...document.querySelectorAll('button:has(svg[data-testid="MenuIcon"])')
        ];
      } else if (step === 3) {
        targetElements = [
          ...document.querySelectorAll('[aria-label*="Create"]'),
          ...document.querySelectorAll('button:has(svg[data-testid="AddBoxIcon"])'),
          ...Array.from(document.querySelectorAll('button')).filter(btn => btn.textContent?.includes('Create Document'))
        ];
      }
  ...
  useEffect(() => {
    ...
    if (step === 2) {
      ...
      targetPos = { 
        ...
        y: Math.max(200, Math.min(containerSize.height - 200, containerSize.height / 2 + 50)) 
      };
    } else if (step === 4) {
      targetPos = { 
        x: Math.max(200, Math.min(containerSize.width - 200, containerSize.width / 2 - 100)), 
        y: Math.max(200, Math.min(containerSize.height - 100, containerSize.height - 200)) 
      };
    } else {
  ...
  useEffect(() => {
    ...
    const handleClick = (e) => {
      ...
      if (step === 2) {
        ...
        if (target.querySelector('[data-testid="MenuIcon"]') || 
        ...
        if (target.querySelector('[data-testid="AddBoxIcon"]') || 
      ...
      >
        ...
        <motion.div
          ...
          animate={{ 
            opacity: 1, 
            scale: 1,
            x: Math.max(50, Math.min(containerSize.width - 150, jellyfishPos.x - 50)),
            y: Math.max(50, Math.min(containerSize.height - 150, jellyfishPos.y - 50))
          }}
          transition={{ 
