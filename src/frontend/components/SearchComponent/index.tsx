...
import useSearchFiles from "./SearchFilesContent";
...
const search_options = [
  ...
  // },
  {
    title: (
      <OptionItem
        title={"Search using regular expression"}
        icon={<AcUnitIcon />}
      />
    ),
    value: "regex",
...
function SearchPopper() {
  ...
  const handleShortcutKeyPress = React.useCallback((event: KeyboardEvent) => {
    if (
      event.key === "f" &&
      (event.metaKey || event.ctrlKey) &&
      !event.shiftKey
    ) {
      dispatch({ type: "SEARCH_TOOL" });
