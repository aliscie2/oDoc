...
const Dashboard = () => {
  ...
  const handleChatSend = async (message) => {
    ...
    setChatHistory(prev => [...prev, { type: 'user', message, id: messageId }]);
    setIsLoading(true);
    ...
    setTimeout(() => {
      ...
      if (lowerMessage.includes('stop') && (lowerMessage.includes('search') || lowerMessage.includes('job'))) {
      ...
      else if (lowerMessage.includes('add') && lowerMessage.includes('event') && lowerMessage.includes('david')) {
        const newEvent = { title: 'Meeting with David', time: 'June 15, 9:00 AM' };
      ...
      else if (lowerMessage.includes('remove') && lowerMessage.includes('automated testing')) {
        aiResponse = 'Removed the "Automated Testing with AI" project from your dashboard';
  ...
  return (
    <Box sx={{ 
      ...
      <Box display="grid" gridTemplateColumns="repeat(auto-fit, minmax(300px, 1fr))" gap={3} mb={4}>
        <JobMatchesCard
          matches={[
            { name: 'Senior React Developer', rate: '$85/hr', color: '#00d4ff' },
        ...
        <AlertsCard
          alerts={[
            { title: 'Payment Dispute - Project Alpha', description: 'Client disputed milestone payment' }
          ]}
