...
const JobUpdateAnimation = ({ currentJob, jobs }) => {
  ...
  const compareValues = (oldVal, newVal, field) => {
    const arrayFields = ["skills", "education", "links", "experience", "certifications", "job_titles", "emails", "contacts"];
  ...
  const getFieldName = (field) => {
    const names = {
      skills: "Skills", education: "Education", links: "Links", 
      experience: "Experience", certifications: "Certifications",
      job_titles: "Job Titles", emails: "Emails", contacts: "Contacts",
      description: "Description", proficiency_level: "Level",
      trust_score: "Trust Score", trust_note: "Trust Note",
  ...
  useEffect(() => {
    ...
    const fieldsToCheck = ["skills", "education", "links", "experience", "certifications", "job_titles", "emails", "contacts", "description", "proficiency_level", "trust_score", "trust_note", "required_match_score"];
    
    const changes = fieldsToCheck
      .map(field => {
        const comparison = compareValues(previousJob[field], currentJob[field], field);
        return comparison.type === "array" 
          ? (comparison.added.length > 0 || comparison.removed.length > 0) ? { field, ...comparison } : null
          : previousJob[field] !== currentJob[field] ? { field, ...comparison } : null;
      })
      .filter(Boolean);
                ...
                <AnimatePresence>
                  {currentUpdate.current.map((item, index) => {
                    ...
                    return (
                      <motion.div
                        key={item}
                        layout
                        initial={isNew ? { opacity: 0, x: -10, scale: 0.95 } : false}
                        ...
                        style={{
                          ...
                          borderRadius: '6px',
                          fontSize: '12px',
                          backgroundColor: isNew ? 'rgba(34, 197, 94, 0.2)' : 'rgba(255, 255, 255, 0.1)',
                          border: `1px solid ${isNew ? 'rgba(34, 197, 94, 0.4)' : 'rgba(255, 255, 255, 0.2)'}`,
                          color: isNew ? '#22c55e' : 'inherit'
                        }}
                      >
                        {isNew && <span style={{ color: '#22c55e', marginRight: '6px' }}>+</span>}
                        {item}
                      </motion.div>
                    );
                  })}
                  ...
                  {currentUpdate.removed.map((item, index) => (
                    <motion.div
                      ...
                      transition={{ duration: 0.6, delay: index * 0.05, times: [0, 0.3, 1] }}
                      style={{
                        padding: '8px 12px',
                        borderRadius: '6px',
                        fontSize: '12px',
