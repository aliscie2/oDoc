...
const getLocation = (job: Job) => {
  ...
  const shouldShowAdditionalFields = () => {
    return highlightedFields.has('skills') || 
  ...
  const getAdditionalHighlightedFields = () => {
    ...
    if (highlightedFields.has('active')) {
      additionalFields.push({
        ...
        icon: currentJob!.active ? <CheckCircle fontSize="small" /> : <Cancel fontSize="small" />
      });
    }
...
const JobBriefData: React.FC<{ 
  ...
  const getJobType = (job: Job) => {
    return job.jobType ? Object.keys(job.jobType)[0] || "Not specified" : "Not specified";
  };
  ...
  const getExperienceLevel = (job: Job) => {
    return job.experienceLevel ? Object.keys(job.experienceLevel)[0] || "Not specified" : "Not specified";
  };
  ...
  const getApplicationDeadline = (job: Job) => {
    if (job.applicationDeadline) {
      return new Date(Number(job.applicationDeadline) / 1000000).toLocaleDateString();
    }
  ...
  const getArrayChanges = (current: string[], previous: string[]) => {
    ...
    return { added, removed, unchanged: current.filter(item => previous.includes(item)) };
  };
  ...
  const getTextChanges = (current: string, previous: string) => {
    ...
    const removed = previousWords.filter(word => !currentWords.includes(word));
  ...
  const renderSkillsWithChanges = () => {
    ...
    const { added, removed, unchanged } = getArrayChanges(currentSkills, previousSkills);
  ...
  const renderTextWithChanges = (current: string, previous: string, fieldName: string) => {
              ...
              {getAdditionalHighlightedFields().map((fieldData) => (
                ...
                >
                  ...
                  <Box sx={{ minWidth: 0, flex: 1 }}>
                    ...
                    >
                      {fieldData.isSpecial ? fieldData.value : (
          ...
          >
            ...
            >
              {highlightedFields.has('description') && previousJob 
                ? renderTextWithChanges(currentJob.description, previousJob.description || "", 'description')
                : currentJob.description
              }
            </Typography>
          </Box>
