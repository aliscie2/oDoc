...
export const useJobMatching = (currentJob: Job | null) => {
  ...
  const processAIMatches = useCallback(
    (aiMatches: AIMatchResponse[], candidateJobs: Job[], currentJobId: string): Match[] => {
      const jobIds = new Set(candidateJobs.map((j) => j.id));
      const uniqueMatches = new Map<string, Match>();
  
      aiMatches.forEach((aiMatch) => {
        if (aiMatch.candidate_id && 
            aiMatch.candidate_id !== currentJobId && 
            jobIds.has(aiMatch.candidate_id)) {
      ...
      console.log("processAIMatches - result:", result.map(m => ({ job_id: m.job_id, score: m.score })));
      return result;
    },
    []
  );
  const findMatches = useCallback(async () => {
    ...
    try {
      ...
      if (import.meta.env.VITE_DFX_NETWORK === "local") {
        ...
        const aiMatchResponses = mockResponse.matches.map((match): AIMatchResponse => ({
        ...
        processedMatches = processAIMatches(aiMatchResponses, candidateJobs, currentJob.id);
      } else {
