...
export function mockJobMatchResponse(
  ...
  const matches: Match[] = potentialMatches.map((candidateJob) => {
    const candidateSkills = new Set(
      (candidateJob.skills || []).map((s) => s.toLowerCase()),
    );
  ...
  console.log(
    "Mock scores generated:",
    matches.map((m) => ({
      id: m.job_id,
      score: m.score,
      required: currentJob.required_match_score,
    })),
  );
  return { matches };
}
