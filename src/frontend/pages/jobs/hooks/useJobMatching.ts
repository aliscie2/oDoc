...
export const useJobMatching = (currentJob: Job | null) => {
  ...
  const processAIMatches = useCallback(
    (aiMatches: AIMatchResponse[], candidateJobs: Job[]): Match[] => {
      ...
      aiMatches.forEach(({ candidate_id, score, missmatching_skills, cover_letter }) => {
  ...
  const findMatches = useCallback(async () => {
    ...
    try {
      ...
      dispatch({ type: "UPDATE_AI_CREDITS", remainingCredits: aiJobMatchResponse.Ok.remaining_credits });
