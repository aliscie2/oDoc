...
const ContractCard: React.FC<ContractCardProps> = ({ contract, profile, allFriends }) => {
  ...
  const promisesAmount = contract.promises?.reduce((sum: number, p: CPayment) => sum + p.amount, 0) || 0;
  const paymentsCount = contract.payments?.length || 0;
  const paymentsAmount = contract.payments?.reduce((sum: number, p: CPayment) => sum + p.amount, 0) || 0;
  const unseenCount = contract._unseenCount;

  const promisesForYou = contract.promises?.filter((p: CPayment) => p.receiver.toString() === profile.id) || [];
  const paymentsForYou = contract.payments?.filter((p: CPayment) => p.receiver.toString() === profile.id) || [];
      ...
      <Box sx={{ position: "relative" }}>
        ...
        <Card
          sx={{
            ...
            border: unseenCount > 0 ? `2px solid ${theme.palette.error.main}` : `1px solid ${theme.palette.divider}`,
            boxShadow: unseenCount > 0 
        ...
        >
          <CardContent sx={{ p: { xs: 2, sm: 2.5 } }}>
            {isShared && (promisesForYou.length > 0 || paymentsForYou.length > 0) && (
              <Box sx={{
                ...
                <Stack direction="row" spacing={1} flexWrap="wrap" useFlexGap>
                  {promisesForYou.length > 0 && (
                    <Chip
                      icon={<HandshakeIcon sx={{ fontSize: 14, color: "inherit !important" }} />}
                  ...
                  {paymentsForYou.length > 0 && (
                    <Chip
                      icon={<CheckCircleIcon sx={{ fontSize: 14, color: "inherit !important" }} />}
            ...
            <Box sx={{ mb: 2 }}>
              ...
              <Chip
                label={isShared 
                  ? allFriends.find(f => f.id === contract.creator)?.name || "Shared" 
                  : "Your Contract"}
                size="small"
                sx={{
                  bgcolor: isShared ? theme.palette.info.main : theme.palette.primary.main,
                  color: "white",
                  fontWeight: 500,
                  fontSize: "0.7rem",
                  height: 22,
            ...
            <Stack spacing={1.5}>
              <Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "center", p: 1.5, bgcolor: "action.hover", borderRadius: 1 }}>
                <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
                  <HandshakeIcon sx={{ color:'white', fontSize: 18, color: "primary.main" }} />
                  <Typography variant="body2" fontWeight={500}>Promises</Typography>
                </Box>
                <Box sx={{ textAlign: "right" }}>
                  <Typography variant="body2" fontWeight={600}>{promisesCount}</Typography>
                  <Typography variant="caption" color="text.secondary">${promisesAmount}</Typography>
                </Box>
              </Box>
              ...
              <Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "center", p: 1.5, bgcolor: "action.hover", borderRadius: 1 }}>
                ...
                <Box sx={{ textAlign: "right" }}>
                  <Typography variant="body2" fontWeight={600}>{paymentsCount}</Typography>
                  <Typography variant="caption" color="text.secondary">${paymentsAmount}</Typography>
                </Box>
              </Box>
            </Stack>
          </CardContent>
...
const ContractsHistory: React.FC = () => {
  ...
  const { profile, all_friends } = useSelector((state: RootState) => state.filesState);
  ...
  const handleCreateContract = () => {
    ...
    dispatch({ 
      type: "ADD_CONTRACT", 
      contract: { ...custom_contract, id: randomString(), creator: profile.id, date_created: Date.now() * 1e6 } 
    });
  };

  if (!profile) {
    return (
      <Box sx={{ display: "flex", alignItems: "center", justifyContent: "center", minHeight: "50vh", m: 2 }}>
        <Typography variant="h6" color="text.secondary">Please login to continue</Typography>
      </Box>
    );
  }
  ...
  return (
    <Box sx={{ maxWidth: 1200, mx: "auto", px: { xs: 2, sm: 3 }, pb: 4 }}>
      ...
      <Box sx={{ mt: { xs: 10, sm: 4 }, mb: 4 }}>
        <Box sx={{ display: "flex", justifyContent: "space-between", alignItems: { xs: "flex-start", sm: "center" }, flexDirection: { xs: "column", sm: "row" }, gap: 2, mb: 3, p: { xs: 2, sm: 3 }, bgcolor: "background.paper", borderRadius: 2, boxShadow: 1 }}>
          <Box>
            <Typography variant="h4" sx={{ fontWeight: 700, fontSize: { xs: "1.5rem", sm: "2rem" } }}>
              Agreements
            </Typography>
            <Typography variant="body2" color="text.secondary">
              {contractsList.length} contract{contractsList.length !== 1 ? "s" : ""}
            </Typography>
          </Box>
          <Button onClick={handleCreateContract} variant="contained" startIcon={<AddIcon sx={{color:'white'}} />} sx={{ fontWeight: 600, whiteSpace: "nowrap", minWidth: "auto" }}>
            New Contract
          </Button>
        </Box>
      </Box>
      ...
      {contractsList.length === 0 ? (
        <Box sx={{ textAlign: "center", py: 8, border: "2px dashed", borderColor: "divider", borderRadius: 2 }}>
          ...
          <Typography variant="h6" sx={{ mb: 1, fontWeight: 600 }}>No contracts yet</Typography>
          <Typography variant="body2" color="text.secondary" sx={{ mb: 3 }}>
            Create your first contract to get started
          </Typography>
          <Button onClick={handleCreateContract} variant="outlined" startIcon={<AddIcon sx={{color:'white'}}/>}>
            Create Contract
          </Button>
        </Box>
      ) : (
        <Grid container spacing={{ xs: 2, sm: 3 }}>
          {contractsList.map(contract => (
            <Grid key={contract.id} size={{ xs: 12, sm: 6, lg: 4 }}>
              <ContractCard contract={contract} profile={profile} allFriends={all_friends} />
            </Grid>
          ))}
        </Grid>
      )}
