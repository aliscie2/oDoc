...
const FeatureSection = ({ title, icon, children, reversed = false, sectionIndex, visibleSection }) => {
  ...
  return (
    ...
    >
      <Grid container spacing={6} direction={reversed && !isMobile ? 'row-reverse' : 'row'} alignItems="center">
    ...
    <Box sx={{ minHeight: '100vh' }}>
      ...
      <FeatureSection 
        ...
        <FeatureSection 
          ...
          icon={<EmotionalAnimation size={isMobile?'sm':"md"} title={"From 40 Hours of Job Hunting to 5 Minutes of AI Magic"} type="Searching" />}
          sectionIndex={1}
          visibleSection={visibleSection}
        >
          <Typography variant="h6" paragraph color="text.secondary" sx={{ mb: 3 }}>
            You can talk to the oDoc AI job match - It will provide consultation to improve your resume/requirements post
          </Typography>
        ...
        <FeatureSection 
          ...
          icon={<EmotionalAnimation size={isMobile?'sm':"md"} type="handShake" />}
          reversed
          sectionIndex={2}
          visibleSection={visibleSection}
        >
          <List sx={{ mb: 3 }}>
            {[
              { primary: 'Escrow', secondary: 'You can make Escrow by deposit before making any promise, to prove you will pay' },
              { primary: 'Promise', secondary: 'You can make non escrow promise for simple cases' },
              { primary: 'Release', secondary: 'You can release when task is done' },
              { primary: 'Karma', secondary: 'When you make cancellations your score will go down, shows you have lower success rate' }
            ].map((item, index) => (
          ...
          <Typography variant="body2" sx={{ fontStyle: 'italic', color: 'text.secondary' }}>
            Eliminate middlemen, spreadsheets, and task managers. Streamline contracts, payments, escrow, and collaboration into one seamless crypto-native platform. Powered by AI & Internet Computer
          </Typography>
        </FeatureSection>
            ...
            <Grid item xs={12} md={6}>
              <Card sx={{ 
                ...
                <CardContent sx={{ p: 4 }}>
                  ...
                  <List dense>
                    {['Repeated cancellations', 'Excessive disputes', 'Breaking contract terms'].map((text, index) => (
                      <ListItem key={index}>
                        <ListItemText 
                          primary={text} 
                          sx={{ 
                            color: theme.palette.mode === 'dark' ? 'text.primary' : 'error.dark'
                          }}
                        />
                      </ListItem>
                    ))}
                  ...
                  <List dense>
                    {['Trust score drops', 'Funds locked', 'Transaction cap'].map((text, index) => (
                      <ListItem key={index}>
                        <ListItemText 
                          primary={text} 
                          sx={{ 
            ...
            <Grid item xs={12} md={6}>
              <Card sx={{ 
                ...
                <CardContent sx={{ p: 4 }}>
                  ...
                  <List dense>
                    {['Releasing payments', 'Creating contracts', 'Interacting with many users', 'High transaction volume'].map((text, index) => (
                      <ListItem key={index}>
                        <ListItemText 
                          primary={text} 
                          sx={{ 
                            color: theme.palette.mode === 'dark' ? 'text.primary' : 'success.dark'
                          }}
                        />
                      </ListItem>
                    ))}
                  ...
                  <List dense>
                    {['Higher trust score', 'Transaction freedom', 'Refund old escrow'].map((text, index) => (
                      <ListItem key={index}>
                        <ListItemText 
                          primary={text} 
                          sx={{ 
        ...
        <FeatureSection 
          ...
          icon={<EmotionalAnimation size={isMobile?'sm':"md"} type="MultiTask" />}
          sectionIndex={isMobile ? 5: 6}
          visibleSection={visibleSection}
        >
          <Typography variant="h6" paragraph color="text.secondary" sx={{ mb: 3 }}>
            The crypto agreement is a table that acts like contract and tasks manager all at once.
          </Typography>
          <List sx={{ '& .MuiListItem-root': { pl: 0 } }}>
            {[
        ...
        <FeatureSection 
          ...
          icon={<EmotionalAnimation size={isMobile?'sm':"md"} type="Security" />}
          ...
          <List sx={{ '& .MuiListItem-root': { pl: 0 } }}>
            {[
              ...
              <ListItem key={index}>
                ...
                <ListItemText 
                  primary={
                    <Typography variant="body1" fontWeight={text.includes('Tamper-proof') || text.includes('Fraud') ? 'bold' : 'normal'}>
                      {text}
                    </Typography>
                  }
                />
