...
test("Test custom contract update permissions", async () => {
  ...
  let init_date: { Ok: InitialData } | { Err: string } =
  ...
  res = await global.actor.multi_updates([], [], [to_store], [], []);
  expect({
    Ok: "Error: You don't have permission to update this cell Updates applied successfully.",
  }).toStrictEqual(res);
  ...
  expect(
    init_date.Ok.Contracts[0][1].CustomContract.contracts[0].rows[0].cells[0]
      .value,
  ).toBe("INIT VALUE");
...
test("Test custom contract view permissions", async () => {
  ...
  expect(
    get_contract.Ok.CustomContract.contracts[0].rows[0].cells[0].value,
  ).toBe("INIT VALUE");
  ...
  expect(get_contract.Ok.CustomContract.contracts[0].rows[0].cells.length).toBe(
    0,
  );
