...
test("Test update cell", async () => {
  ...
  res = await global.actor.multi_updates([], [], [to_store], [], []);
  expect(res).toEqual({
    Ok: "Error: You don't have permission to update column: Untitled Updates applied successfully.",
  });
  ...
  expect(
    res.Ok.Contracts[0][1].CustomContract.contracts[0].rows[0].cells[0].value,
  ).toEqual("NEW VALUE");
});
