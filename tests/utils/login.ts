...
export const registerUser = async (page) => {
 ...
 await page1.getByRole('textbox', { name: 'Type the characters you see' }).click();
 ...
 await page1.waitForEvent('close');
  await page.waitForLoadState('networkidle');
  await page.getByRole('textbox', { name: 'Username' }).fill(Math.random().toString(36).substring(7));
  await page.getByRole('textbox', { name: 'Bio' }).fill(Math.random().toString(36).substring(7));
