...
const dfxJson = JSON.parse(readFileSync(path.resolve(__dirname, "dfx.json"), "utf8"));
...
function initCanisterIds() {
  try {
    localCanisters = JSON.parse(readFileSync(path.resolve(".dfx", "local", "canister_ids.json"), "utf8"));
  } catch (error) {
    console.log("No local canister_ids.json found. Continuing production");
  }
  try {
    prodCanisters = JSON.parse(readFileSync(path.resolve("canister_ids.json"), "utf8"));
    localEnv = false;
  } catch (error) {
...
export default defineConfig({
  ...
  proxy: { 
    "/api": { 
      target: "http://localhost:8080",  // change 8080 to ur dfx port, u can get the port by runing `dfx info webserver-port`
      changeOrigin: true 
    } 
  },
  watch: {
  ...
  resolve: {
    alias: {
      ...
      ...Object.entries(dfxJson.canisters).reduce((acc, [name]) => {
        ...
        const outputRoot = join(__dirname, ".dfx", networkName, "canisters", name);
  ...
  define: {
    global: "globalThis",
    ...Object.entries(canisters || {}).reduce((acc, [key, val]) => ({
      ...acc,
      [`process.env.${key.toUpperCase()}_CANISTER_ID`]: JSON.stringify(isDevelopment ? val.local : val.ic)
    }), {}),
  ...
  // In your vite.config - add this to optimizeDeps
    optimizeDeps: {
      include: ["@emotion/react", "@emotion/styled", "@mui/material/Tooltip", "@mui/x-data-grid-generator"],
      force: true
    }
});